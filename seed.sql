-- 강남룸 블로그 게시글 샘플 데이터
INSERT OR IGNORE INTO posts (title, content, excerpt, author, category, slug) VALUES 
  (
    '강남룸 프리미엄 가라오케 오픈 안내',
    '강남 최고의 프리미엄 가라오케 강남룸이 새롭게 오픈했습니다. 최신 음향시설과 고급스러운 인테리어로 특별한 시간을 보내실 수 있습니다. 문의: 010-5197-1332 (최익현 실장)',
    '강남 최고의 프리미엄 가라오케 강남룸 오픈',
    '최익현',
    '강남룸 소식',
    'gangnam-room-opening'
  ),
  (
    '강남룸 시스템 및 가격 안내',
    '강남룸은 합리적인 가격으로 최고의 서비스를 제공합니다. 각종 이벤트 및 단체 예약 시 특별 할인 혜택을 드립니다. 자세한 상담은 010-5197-1332로 연락주세요.',
    '강남룸 가격 및 시스템 상세 안내',
    '최익현',
    '가격정보',
    'gangnam-room-pricing'
  ),
  (
    '강남룸 예약 및 문의 안내',
    '강남룸 예약은 전화 또는 방문 예약 가능합니다. 주말 및 공휴일은 사전 예약 필수입니다. 최익현 실장 010-5197-1332',
    '강남룸 예약 방법 및 이용 안내',
    '최익현',
    '이용안내',
    'gangnam-room-reservation'
  ),
  (
    '강남 가라오케 강남룸 위치 및 교통편',
    '강남역 도보 5분 거리, 교통이 편리한 강남룸입니다. 지하철 2호선, 신분당선 강남역 이용 가능. 주차 시설 완비.',
    '강남룸 위치 및 오시는 길',
    '최익현',
    '위치정보',
    'gangnam-room-location'
  ),
  (
    '강남룸 VIP 룸 소개',
    '강남룸의 프리미엄 VIP 룸을 소개합니다. 최신 음향장비와 넓은 공간, 고급스러운 인테리어로 특별한 날을 더욱 특별하게 만들어드립니다.',
    '강남룸 VIP 룸 시설 소개',
    '최익현',
    '시설안내',
    'gangnam-room-vip'
  );
